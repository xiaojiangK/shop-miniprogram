...
Page({
    ...
    a.setData({
      ...
      e.get("goods/get_detail", {
        ...
        if (s.wxParse("wxParseData", "html", t.goods.content, a, "0"), a.setData({
          ...
          goods: {
            ...t.goods,
            discounts: t.goods.discounts && JSON.parse(t.goods.discounts),
            dispatchcitys: t.goods.dispatchcitys ? t.goods.dispatchcitys.split(';').filter(item => item) : [],
            edareas: t.goods.edareas ? t.goods.edareas.split(';').filter(item => item) : []
          }
        ...
        t.goods.ispresell > 0 && a.setData({
          ...
          t.goods.getComments > 0 && e.get("goods/get_comments", {
            ...
            a.setData({
              goods: {
                ...a.data.goods,
                preselltimeend: a.data.preselltimeend
              },
              commentObj: t
            })
          })
  ...
  countDown: function (t, e, a) {
    ...
    if (this.setData({
      ...
      const d = Math.floor(leftsecond / (60 * 60 * 24));
      const h = Math.floor((leftsecond - 24 * day1 * 60 * 60) / 3600);
      const m = Math.floor((leftsecond - 24 * day1 * 60 * 60 - 3600 * hour) / 60);
      const s = Math.floor(leftsecond - 24 * day1 * 60 * 60 - 3600 * hour - 60 * minute);
      var l = "";
  ...
  getWidget() {
    e.post("/app/ewei_shopv2_api.php", {
      i: 1,
      c: "site",
      a: "entry",
      m: "Lewei_shopv2",
      ...
      if (t.shop_detail) {
        const detail = t.shop_detail;
        const param = detail[detail.length - 1].param;
        wx.setNavigationBarTitle({
          title: param.title
        });
